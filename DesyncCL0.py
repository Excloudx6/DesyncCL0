...
# This tool will be ran with 23 paralell hosts using the Axiom framework by Pry0cc, so it's important to keep the output minimal and identifyable.
...
def cl0_check(URL, SRL, user_agent, timeout, debug):
    ...
    requestDesync = (
        requestDesync + "Content-Length: " + str(len(requestSmuggled)) + "\r\n"
    )
    requestDesync = requestDesync + "Connection: keep-alive\r\n"
    requestDesync = (
        requestDesync + "Content-Type: application/x-www-form-urlencoded\r\n"
    )
    ...
    if debug:
        ...
        print(requestRootSmuggled)
        print("<<<<< requestRootSmuggled")
    httpresponseRootSmuggled, bodyRootSmuggled = send_request(
        sock, requestRootSmuggled.encode("utf-8")
    )
    sock.close()
    if debug:
    ...
    if (
        httpresponseRootSmuggled.status == httpresponse404.status
        and httpresponseRootSmuggled.status != httpresponseRoot.status
    ):
        print(
            "\r\n-=---------------------------=-\r\n"
            + URL.scheme
            ...
            + URL.path
            + "\r\n"
            + "Potential CL.0 Found! Payload:\r\n"
            + "\r\n"
            + requestDesync
            # + requestDesync.replace("\r\n", "\\r\\n")
            + "\r\n-=---------------------------=-"
        )

        # print(
        #     "Potential Cl.0 Found! Back-end server interpreted request as start of another request"
        #     + URL.scheme
        #     + "://"
        #     + URL.netloc
        #     + URL.path
        ...
        httpresponseRootSmuggled.status == httpresponseRoot.status
        and httpresponseRootSmuggled.status == httpresponse404.status
        and str(httpresponseRootSmuggled.status).startswith("3")
        and httpresponseRootSmuggled.getheader("Location")
        != httpresponseRoot.getheader("Location")
    ):
        ...
        print(
            "httpresponseRootSmuggled", httpresponseRootSmuggled.getheader("Location")
        )
        ...
        print(
            "WARNING! All responses are redirects to the same location.",
            httpresponseRootSmuggled.getheaders(),
        )
        print(
            "Try to debug with an invalid or HEAD method on the smuggled request line."
        )
    else:
        print(URL.scheme + "://" + URL.netloc + URL.path + " is not vulnerable.")
...
def check_url(url):
    ...
    if ((url_checked.scheme != "http") & (url_checked.scheme != "https")) | (
        url_checked.netloc == ""
    ):
        raise argparse.ArgumentTypeError(
            "Invalid %s URL (example: https://www.example.com/path)." % url
        )
    return url_checked
...
def Desync():
    ...
    # Use minimal print statements since ,",

    parser = argparse.ArgumentParser(
        prog="DesyncCL0", description="Detects HTTP desync CL.0 vulnerabilities."
    )
    ...
    parser.add_argument(
        ...
        help='Set default User-Agent request header (default "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/104.0.0.0 Safari/537.36").',
    )
    parser.add_argument(
        "-d",
        "--debug",
